...
public class MarchingSquares : MonoBehaviour
{
    ...
    {
        xOff += speed.x * Time.deltaTime;
        yOff += speed.y * Time.deltaTime;
    ...
    void GenerateNewChunkData()
    {
        ...
        for(int x = 0; x < xSize+2; x++){
            for(int y = 0; y < ySize+2; y++){
                ...
                {
                    float perlinX = (chunkOffset.x * xSize + x + xOff) / scale * frequency;
                    float perlinY = (chunkOffset.y * ySize + y + yOff) / scale * frequency;
