...
public class WorldController : MonoBehaviour
{
    ...
    void Start(){
        for(int i = -1; i <= 1; i++){
            for(int j = -1; j <= 1; j++){
                MarchingSquares newChunk = Instantiate(chunkPrefab.gameObject, new Vector3(i * chunkSize - (chunkSize * 0.5f), j * chunkSize - (chunkSize * 0.5f), 0f), Quaternion.identity).GetComponent<MarchingSquares>();
